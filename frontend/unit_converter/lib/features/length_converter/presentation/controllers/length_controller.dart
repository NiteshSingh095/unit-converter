...
class LengthController extends GetxController {
  LengthController(this._convertLengthUseCase);

  final ConvertLengthUseCase _convertLengthUseCase;

  final valueController = TextEditingController();
  final formKey = GlobalKey<FormState>();

  final units = UnitLists.lengthUnits;
  final fromUnit = 'm'.obs;
  final toUnit = 'ft'.obs;
  final isLoading = false.obs;
  final errorMessage = RxnString();
  final conversion = Rxn<LengthConversion>();
  ...
  Future<void> convert() async {
    ...
    final result = await _convertLengthUseCase(
      value: value,
      from: fromUnit.value,
      to: toUnit.value,
    );
